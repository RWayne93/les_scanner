...
def format_rank(rank: str) -> Union[Tuple[str, str], str]:
  if rank[1:].isdigit() and len(
      rank) == 2:  # If the rank is of the form "EX" or "WX" or "OX"
    return rank[0] + '0' + rank[1], rank[0] + '-' + rank[1]  # Returns "E0X", "E-X" or "W0X", "W-X" or "O0X", "O-X"
  else:
    return rank

def check_bas(rank: str, entitlements: dict):
...
def check_pay(entitlements: dict, rank: str, years: int, mha: dict):
  base_pay_str = base_pay_data[base_pay_data['rank'] == format_rank(rank)[1]][format_years(years)].values[0]
  ...
  bah_without_dependents = data1[data1['MHA'] ==
                                 mha][format_rank(rank)[0]].values[0]
